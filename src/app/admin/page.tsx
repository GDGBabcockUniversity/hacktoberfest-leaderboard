...
export default async function Admin({
  ...
  if (!(await isAdmin()))
    return <Login failed={(await searchParams).error === "invalid"} />;
  const { people, rounds } = await snapshot();
  const watchedRepos = JSON.parse(
    (
      await db
        .select()
        .from(syncState)
    ...
    <main>
      ...
      <Card className="admin-panel mb-8 p-5">
        ...
        <p className="mt-2 muted">
          Enter a repository name or a full <code>organisation/repository</code>{" "}
          name.
        </p>
        <form action={addWatchedRepo} className="mt-3 flex flex-wrap gap-2">
          <Input
            name="repo"
            placeholder="repository-name or org/repository"
            required
            className="w-auto"
          />
        ...
        <p className="mt-2 muted">
          Deleting a repository also removes its synced pull requests from the
          leaderboard.
        </p>
        <div className="mt-3 flex flex-col gap-2">
          ...
          {!watchedRepos.length && (
            <p className="muted">No watched repositories yet.</p>
          )}
        </div>
      </Card>
      <Card className="admin-panel mb-8 p-5">
        ...
        <p className="mt-2 muted">
          Deleting a contributor removes their synced pull requests and trivia
          scores. A later GitHub sync may add them again.
        </p>
        <div className="mt-3 flex flex-col gap-2">
          {people
            ...
            .map((person) => (
              ...
              >
                <span className="min-w-0 break-all">
                  {person.displayName}{" "}
                  <span className="muted">@{person.githubUsername}</span>
                </span>
                <form
                  action={removeContributor.bind(null, person.githubUsername)}
                >
                  <Button
                    type="submit"
                    variant="destructive"
                    size="sm"
                    aria-label={`Delete contributor ${person.githubUsername}`}
                  >
                    Delete contributor
                  </Button>
                </form>
          ...
          {!people.some((person) => person.id) && (
            <p className="muted">No synced contributors yet.</p>
          )}
        </div>
      </Card>
      <Card className="admin-panel mb-8 p-5">
